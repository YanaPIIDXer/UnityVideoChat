...
namespace Photon.Realtime.Editor
{
    ...
    public partial class WizardWindow : EditorWindow
    {
        ...
        private GUIStyle stepBoxActiveStyle;
        ...
        private void InitContent()
        {
            ...
            this.stepBoxActiveStyle = new GUIStyle(this.stepBoxStyle)
            {
        ...
        private void DrawNavMenu()
        {
            ...
            this.DrawStepOption(this.introIcon, WizardText.STEP_INTRO, active: this.currentStage == WizardStage.Intro, callback: () =>
            {
                this.SetStep(WizardStage.Intro);
            });

            this.DrawStepOption(this.documentationIcon, WizardText.STEP_HISTORY, active: this.currentStage == WizardStage.ReleaseHistory, callback: () =>
            {
                this.SetStep(WizardStage.ReleaseHistory);
            });

            this.DrawStepOption(this.photonCloudIcon, WizardText.STEP_PHOTON, active: this.currentStage == WizardStage.Photon, callback: () =>
            {
                this.SetStep(WizardStage.Photon);
            });

            this.DrawStepOption(this.discordIcon, WizardText.STEP_SUPPORT, active: this.currentStage == WizardStage.Support, callback: () =>
            {
        ...
        private void DrawStepOption(Texture2D icon, string header, string description = null, bool? active = null, Action callback = null)
        {
            var content = new GUIContent()
            {
                text = description == null ? header : string.Format("{0}\n{1}", header, description), // TODO: Remove format
                image = icon,
            };

            var renderStyle = active.HasValue && active.Value == true ? this.stepBoxActiveStyle : this.stepBoxStyle;

            if (GUILayout.Button(content, renderStyle) && callback != null)
            {
                callback.Invoke();
            }
        }
